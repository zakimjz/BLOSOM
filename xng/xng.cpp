...
public:
	...
	void show( bool only_items, bool fLetters, bool fOutput ){
		...
		cout << endl << endl << "XNG Classes:" << endl << endl;
...
int main(int argc, char *argv[])
{
	...
	if( fOutput ){ db.show( fLetters ); XNGs.fPrint1 = false; }

	XNGs.getClasses( db, min_sup, max_sup, max_item, fNegative ); 
	XNGs.classes.show( fOnlyItems, fLetters, fOutput );
